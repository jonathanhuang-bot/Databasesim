...
impl Join {
    ...
    pub fn new(
        ...
        let leftplusright = left_child.get_schema().merge(right_child.get_schema());
        let predicate = JoinPredicate::new(op, left_index, right_index);
        Self {
            predicate,
            left_child,
            right_child,
            schema: leftplusright,
...
impl OpIterator for Join {
    fn open(&mut self) -> Result<(), CrustyError> {
        if self.current_left_tuple.is_some() {
            return Err(CrustyError::CrustyError(String::from("cannot open")));
        }
    ...
    fn next(&mut self) -> Result<Option<Tuple>, CrustyError> {
        if self.current_left_tuple.is_none() {
            panic!("Cannot call next since OpIterator was never opened");
        }

        while let Some(left_tuple) = self.current_left_tuple.clone() {
            if let Some(right_tuple) = self.right_child.next()? {
                let left_field = left_tuple.get_field(self.predicate.left_index);
                let right_field = right_tuple.get_field(self.predicate.right_index);
                ...
                // End of the right child, rewind and move to the next left tuple
                self.right_child.rewind()?;
                self.current_left_tuple = self.left_child.next()?;
            }
        }

        // End of the left child, return None
        Ok(None)
    }

    fn close(&mut self) -> Result<(), CrustyError> {
        if self.current_left_tuple.is_none() {
            panic!("cannot be closed since Opiterator was never opened");
        }
...
pub struct HashEqJoin {
    ...
    schema: TableSchema,
    hashmap: HashMap<Field, Vec<Tuple>>,
}
...
impl OpIterator for HashEqJoin {
    fn open(&mut self) -> Result<(), CrustyError> {
        self.left_child.open()?;
        self.right_child.open()?;
        while let Ok(Some(left_tuple)) = self.left_child.next() {
            let key = left_tuple
                .get_field(self.predicate.left_index)
                .cloned()
                .unwrap();
            self.hashmap.entry(key).or_default().push(left_tuple);
        }
        self.left_child.rewind()
    }

    fn next(&mut self) -> Result<Option<Tuple>, CrustyError> {
        /* let mut hashmap: HashMap<Field, Vec<Tuple>> = HashMap::new();

        while let Some(left_tuple) = self.left_child.next()? {
            let key = left_tuple.get_field(self.predicate.left_index).cloned().unwrap();
            hashmap.entry(key).or_default().push(left_tuple);
        }
        ...
        while let Some(right_tuple) = self.right_child.next()? {
            ...
            if let Some(left_tuples) = self.hashmap.get(&key) {
                for left_tuple in left_tuples {
                    if self.predicate.op.compare(
                        &left_tuple.get_field(self.predicate.left_index),
                        &right_tuple.get_field(self.predicate.right_index),
                    ) {
                        let merged_tuple = left_tuple.merge(&right_tuple);
                        return Ok(Some(merged_tuple));
                    }
